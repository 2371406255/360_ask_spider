...
class PostArticle(Thread):
    ...
    def generate_form(self, keywords, article):
        ...
        if cms == "ssycms":
            return {
                'password': 'www.ssycms.com',  # 插件密码
                'title': article['title'],
                'content': article['contents'],
                'cid': self.config['catid'],  # 分类id
                'tags': keywords,
            }
        if cms == 'zblog':
            return {
                'ruku_password': '123456',  # 火车头插件入库密码
                'ID': '',
                'CateID': self.config['catid'],
                'Title': article['title'],
                'Tag': keywords,
                'Intro': '',
                'Content': article['contents'],
...
if __name__ == "__main__":
    ...
    post_config = dict(
        dbconf=dbconfig,
        table_name="baidu-juzi",  # 关键词对应的数据库内容表
        catid=1,  # 要发布的分类id
        cms="zblog",  # 要发布的程序名, phpcms, dedecms, empirecms, ssycms, zblog
        backend="http://myblog.com/zb_users/plugin/huochetoumiandenglu/fabu_api.php",  # 发布的后台地址
        queue=word_queue,
